...
# Two text-typed subsets, named because they are not interchangeable:
#   TextRegistryTypes       - data is text, so 'caseSensitive' changes the result.
#                             MultiString qualifies: its elements compare per-element
#                             through the same case-sensitivity rule.
#   ScalarTextRegistryTypes - data is a single string. Only these support the
#                             substring operators; MultiString is a string[] with no
#                             one string to search, so Compare-RegistryValue does not
#                             implement Contains/StartsWith/EndsWith for it.
# Keeping them apart is the point: collapsing them into one list would either let
# Contains through on a MultiString or stop caseSensitive applying to one.
$script:TextRegistryTypes = @('String', 'ExpandString', 'MultiString')
$script:ScalarTextRegistryTypes = @('String', 'ExpandString')
    ...
    try {
        ...
        switch ($Comparison) {
            'Equals' {
                ...
                # String/ExpandString
                elseif ($Type -in $script:ScalarTextRegistryTypes) {
...
function Get-ValueSignature {
    ...
    .DESCRIPTION
        ...
        distinguish two entries. Booleans are read the way the detect/remediate
        code reads them ('-eq $true', not a [bool] cast) so the signature can
        never disagree with the runtime about what a non-boolean JSON value means:
        [bool]'false' is $true, while 'false' -eq $true is $false.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        ...
        # The group's action. DeleteKey never reaches here - it ignores 'values'
        # entirely, so its entries instruct nothing at all.
        [ValidateSet('Set', 'Delete')]
        [string]$Action = 'Set'
    )

    # A Delete group reads nothing but the name - detection checks the value is
    # gone, remediation removes it - so type, data, comparison, caseSensitive and
    # skipDetection carry no instruction and two Delete entries for one name always
    # agree. That is the same instruction a Set value with comparison NotExists
    # expresses, so the two canonicalise to one signature.
    if ($Action -eq 'Delete') {
        return 'comparison=notexists|skipDetection=False'
    }
    ...
    if ($comparison -eq 'NotExists') {
        return "comparison=notexists|skipDetection=$($Value.skipDetection -eq $true)"
    }
    ...
    # caseSensitive only reaches the text-matching comparisons on the string types.
    # Compare-RegistryValue answers Exists from the value's presence alone and never
    # looks at the flag, the numeric operators can't reach a string type at all, and
    # remediation writes the literal data either way - so on anything else the flag
    # is inert and letting it differ would report a conflict between two entries
    # that behave identically.
    $readsCaseSensitive = ($Value.type -in $script:TextRegistryTypes) -and
                          ($comparison -in 'Equals', 'NotEquals', 'Contains', 'StartsWith', 'EndsWith')
    $caseSensitive = if ($readsCaseSensitive) { $Value.caseSensitive -eq $true } else { $false }
    ...
    return @(
        ...
        "skipDetection=$($Value.skipDetection -eq $true)"
...
function Assert-ValidConfig {
    ...
    .DESCRIPTION
        ...
        Problems that make a config impossible to act on throw; problems the engine
        can proceed through emit a warning (Write-Warning, not Write-Log, so the
        function stays usable by the package generator without pulling in the logging
        setup - Get-Configuration captures the warnings and re-emits them through
        Write-Log so they reach the file log and Event Log on a packaged run).
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    # Duplicate value names are tracked across the whole config rather than within a
    # single setting group: nothing stops two groups from naming the same scope+path,
    # and a name declared in both fights in exactly the same way. Keyed
    # scope|path -> (name -> signature).
    #
    # Both levels need an ordinal comparer. A PowerShell @{} literal compares keys
    # with CurrentCultureIgnoreCase, and linguistic collation treats control and
    # format characters (U+00AD, U+200B, ...) as ignorable - so two genuinely distinct
    # value names would collide and a valid config would be rejected. This is the
    # same trap the signature comparison below avoids by using Ordinal.
    $seenByKey = [hashtable]::new([System.StringComparer]::OrdinalIgnoreCase)
    ...
    foreach ($setting in $Config.settings) {
        ...
        if ($setting.values) {
            foreach ($value in @($setting.values)) {
                ...
                if ($value.type -and $value.comparison) {
                    ...
                    if ($value.comparison -in 'Contains', 'StartsWith', 'EndsWith' -and
                        $value.type -notin $script:ScalarTextRegistryTypes) {
                        throw "Value '$($value.name)' under '$($setting.path)' uses string comparison '$($value.comparison)', which requires one of: $($script:ScalarTextRegistryTypes -join ', ') (got '$($value.type)')"
                    }
                }
            }

            # A name listed twice for the same key is either redundant (identical
            ...
            # config through.
            #
            # DeleteKey is exempt: it removes the whole key and ignores 'values'
            # entirely, so a name repeated there instructs nothing and cannot fight
            # anything - including a Set or Delete group naming the same path.
            if ($setting.action -ne 'DeleteKey') {
                # Scope and path both decide which key is written, and both are
                # matched case-insensitively downstream. Trailing separators are
                # trimmed so 'SOFTWARE\X' and 'SOFTWARE\X\' group together.
                $groupKey = "$($setting.scope)|$("$($setting.path)".Trim('\'))"
                if (-not $seenByKey.ContainsKey($groupKey)) {
                    $seenByKey[$groupKey] = [hashtable]::new([System.StringComparer]::OrdinalIgnoreCase)
                }
                $seenValues = $seenByKey[$groupKey]

                foreach ($value in @($setting.values)) {
                    $signature = Get-ValueSignature -Value $value -Action $setting.action
                    ...
                    if ($seenValues.ContainsKey($value.name)) {
                        ...
                        if (-not [string]::Equals($seenValues[$value.name], $signature, [System.StringComparison]::Ordinal)) {
                            Write-Warning "Value '$($value.name)' is declared more than once for $($setting.scope) path '$($setting.path)' with settings that appear to disagree. Remediation applies them in order, so the last one wins and detection will keep failing on the earlier entry - keep only the entry you want."
                        }
                        else {
                            Write-Warning "Value '$($value.name)' is declared more than once for $($setting.scope) path '$($setting.path)' with identical settings - the duplicate has no effect and can be removed."
                        }
                    }
                    else {
...
function Get-Configuration {
    ...
    try {
        ...
        # Assert-ValidConfig warns through Write-Warning so the package generator can
        # share it without the logging setup. On a packaged run that stream goes
        # nowhere - SYSTEM, non-interactive, nobody watching - so capture the
        # warnings and re-emit them through Write-Log, which lands them in the daily
        # file log and the Event Log where a config problem can actually be found
        # per-device after the fact.
        $configWarnings = $null
        Assert-ValidConfig -Config $config -WarningVariable configWarnings -WarningAction SilentlyContinue
        foreach ($configWarning in $configWarnings) {
            Write-Log "Config validation: $configWarning" -Level Warning
        }
