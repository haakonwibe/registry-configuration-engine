...
**Versioning**: all version references move together — `$script:EngineVersion`, the header comment, the README's `{{ENGINEVERSION}}` example, `docs/poster.html` and its preview image — and the tag/release is cut at the same time. Batch several changes into a version rather than bumping per fix. Never reuse a version that has already been tagged.

**New-IntunePackage.ps1**: Reads the sibling `Invoke-RegistryConfigEngine.ps1` and produces two self-contained scripts (`<Prefix>-Detect.ps1`, `<Prefix>-Remediate.ps1`) by replacing the engine's `INJECTION_POINT` region with the embedded config (here-string), `$script:__ForcedMode`, and `$script:__ForcedEventLog = $true`. The engine is the single source of truth — there is no separate template. It also dot-sources the engine (which is dot-source-safe) to reuse `Assert-ValidConfig` for its package-time validation and to read `$script:EngineVersion` directly, rather than re-implementing either. Because the validator comes from the *sibling engine file*, the rules applied at package time are whichever engine sits next to the generator — updating `New-IntunePackage.ps1` alone gains nothing; the two must travel together. The pre-flight (read, here-string guard, parse, validate) runs in a try/catch that reports the reason on one line and exits 1: a rejected config is a user-facing outcome, and under `$ErrorActionPreference = 'Stop'` an uncaught throw would bury the message in a CategoryInfo/FullyQualifiedErrorId dump. Validation precedes all writes, so a rejected config leaves no partial package. Generated scripts pin to the engine version they were built against. `-VerboseLogging` injects `$VerbosePreference = 'Continue'` (standard PS mechanism, not a custom flag).

**ConvertFrom-RegistryExport.ps1**: Converts Windows Registry export (.reg) files to JSON configuration format. Supports all registry value types and automatically maps HKLM→Machine, HKCU→User, HKU\<SID>→User (SID stripped from path), HKU\.DEFAULT→DefaultUser (with a warning — .DEFAULT is technically the LocalSystem profile). A key's default value (`@=` in the .reg) is emitted with the name `(default)`, not an empty string — the registry provider can't bind `-Name ''`. Because it groups by `scope|path` and appends, a `.reg` naming one value twice (concatenated exports, overlapping subtree exports, or a key section repeated) used to merge into a config the engine could never satisfy. `Remove-ConvertedValue` now drops the earlier entry before appending, so the last occurrence wins exactly as regedit resolves it on import; it clears the name from the Set *and* Delete groups, which are separate settings but one registry value. Each drop warns and the summary reports the count. This is the fix that matters — the engine's duplicate check downstream only warns (see the validation bullet below), so keeping duplicates out of the config in the first place is what actually protects the author.
...
- Config validation (all load paths, including Validate mode) rejects unknown `scope`/`action` values and `Set`/`Delete` groups without a non-empty `values` array; `DeleteKey` requires no `values`. It also validates per-value enums: an unknown `type` or `comparison` is rejected, and a `Set` value requires a `type` unless its `comparison` is `NotExists` (which deletes rather than writes, so needs no type/data) — this surfaces typos in Validate mode instead of as a runtime failure (typo'd `type` → permanent non-compliance; typo'd `comparison` → an unhandled binding error mid-detection). It enforces operator/type pairing (numeric operators `GreaterThan`/`LessThan`/… require `DWord`/`QWord`; substring operators `Contains`/`StartsWith`/`EndsWith` require `String`/`ExpandString`) so e.g. a `GreaterThan` on a `String` can't silently become a lexicographic compare. An empty value name is rejected with guidance to use `(default)` for a key's default value (the provider can't bind `-Name ''`). It also rejects paths/value names containing `* ? [ ]` or backtick — the registry provider would treat them as wildcards and silently match nothing (full literal-path support is a documented Future Enhancement in README.md). A value `name` repeated for the same `scope`+`path` **warns and never throws**: remediation applies values in order, so the last one wins and detection then fails on the earlier entry on every run — a loop that never converges, but a visible one. Rejecting it would be the worse trade. Telling two entries apart requires `Get-ValueSignature` to model the whole runtime (type resolution, comparison semantics, defaults, boolean coercion, per-action fields), and any gap in that model surfaces as a *false* conflict — a working config refused with exit code 3 and a package build blocked. The signature is necessarily a second model of the engine's own semantics, so treat any change to comparison or write behaviour as a change to it too - and keep it advisory rather than authoritative. `Get-Configuration` captures the warnings via `-WarningVariable` and re-emits them through `Write-Log`, which is what gets them into the daily file log and Event Log — `Assert-ValidConfig` uses bare `Write-Warning` so the package generator can share it without the logging setup, and that stream goes nowhere on an unattended SYSTEM run. The primary defence is upstream: `ConvertFrom-RegistryExport.ps1` resolves duplicates at conversion time, keeping the last occurrence as regedit does on import (across the Set and Delete groups both, since they target one registry value), so the config never carries the problem into the engine. Tracking is config-wide rather than per setting group — two groups may name the same key — keyed `scope|path` with trailing backslashes trimmed; `DeleteKey` is exempt, since it ignores `values` entirely. Matching details worth keeping: which fields bear an instruction depends on the group's `action` (passed as `-Action`) and the resolved `comparison`, so a `Delete` and a `Set`/`NotExists` of one value canonicalise alike while a `Set` that writes and a `Delete` that removes do not; `caseSensitive` counts only where both the type (`$script:TextRegistryTypes`) and the comparison read it; booleans are read as the runtime reads them (`-eq $true`, never `[bool]` — `[bool]'false'` is `$true`); `data` resolves through `Convert-RegistryValue` so `4294967295`, `-1` and `"0xffffffff"` are one DWord instruction, falling back to the raw JSON form when malformed; MultiString elements are length-prefixed, since no character is guaranteed absent from a `REG_MULTI_SZ` element. Everything compares ordinally — signatures with `[string]::Equals(..., Ordinal)` and both levels of the tracking map with `[hashtable]::new([StringComparer]::OrdinalIgnoreCase)`, **not** a `@{}` literal. PowerShell's `-ceq` and the default hashtable comparer are both culture-sensitive, and linguistic collation treats control and format characters (U+001F, U+00AD, U+200B, …) as ignorable, which merges `ab` with `a<U+00AD>b` — two genuinely distinct registry values. All of this lives in one function, `Assert-ValidConfig` (canonical enums in `$script:ValidRegistryTypes` / `$script:ValidComparisonOperators`; the two text-typed subsets in `$script:TextRegistryTypes` and `$script:ScalarTextRegistryTypes`, kept separate on purpose — `caseSensitive` applies to all three text types, but only the scalar two support the substring operators, since a `MultiString` is a `string[]` with no single string to search): `Get-Configuration` calls it at runtime load, and `New-IntunePackage.ps1` dot-sources the engine and calls the same function for its package-time pre-flight — single source of truth, no mirrored copy to drift. Detection wraps each path's evaluation in a try/catch (mirroring remediation), so a value that slips past validation is marked non-compliant instead of aborting the whole run with a ConfigError.
