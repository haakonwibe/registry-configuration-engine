...
**Versioning**: `$script:EngineVersion` and the README's `{{ENGINEVERSION}}` example track `main` and are bumped with the change. `docs/poster.html` and its preview image advertise the newest *release*, so they are updated when a tag is cut, not on every bump — this avoids regenerating the ~770 KB preview PNG for changes nobody has downloaded yet. Once a version is tagged, never reuse it: main moves to the next patch number instead.
...
- Config validation (all load paths, including Validate mode) rejects unknown `scope`/`action` values and `Set`/`Delete` groups without a non-empty `values` array; `DeleteKey` requires no `values`. It also validates per-value enums: an unknown `type` or `comparison` is rejected, and a `Set` value requires a `type` unless its `comparison` is `NotExists` (which deletes rather than writes, so needs no type/data) — this surfaces typos in Validate mode instead of as a runtime failure (typo'd `type` → permanent non-compliance; typo'd `comparison` → an unhandled binding error mid-detection). It enforces operator/type pairing (numeric operators `GreaterThan`/`LessThan`/… require `DWord`/`QWord`; substring operators `Contains`/`StartsWith`/`EndsWith` require `String`/`ExpandString`) so e.g. a `GreaterThan` on a `String` can't silently become a lexicographic compare. An empty value name is rejected with guidance to use `(default)` for a key's default value (the provider can't bind `-Name ''`). It also rejects paths/value names containing `* ? [ ]` or backtick — the registry provider would treat them as wildcards and silently match nothing (full literal-path support is a documented Future Enhancement in README.md). A value `name` repeated within one setting group is rejected when the entries disagree and warned about when they are identical: remediation applies values in order, so the last one wins and detection then fails on the earlier entry on every run — a remediation loop that never converges. Duplicates arrive by way of `ConvertFrom-RegistryExport.ps1`, which groups by `scope|path` and appends values, so a `.reg` containing the same key section twice (concatenated exports, or overlapping subtree exports) merges into one group with the name repeated. Entries are matched by `Get-ValueSignature`, which covers the fields that decide what gets written and how it is detected (`type`, `data`, `comparison`, `skipDetection`, `caseSensitive`) with the engine's own defaults applied, so an omitted optional field matches an explicit one. `data` is resolved through `Convert-RegistryValue` first — otherwise `4294967295`, `-1` and `"0xffffffff"` read as three different DWord instructions when all three write the identical value, and the author is told their entries disagree when they do not. That is reachable in practice: `ConvertFrom-RegistryExport.ps1` emitted `-1` before v1.2.2 and the unsigned decimal after, so concatenating an old converted config with a new one produces exactly that pair. Malformed data falls back to the raw JSON form (the runtime converter reports it better than a duplicate-check failure would). `NotExists` ignores `type`/`data` entirely, since it deletes rather than writes. MultiString elements are length-prefixed rather than joined on a separator — no character is guaranteed absent from a `REG_MULTI_SZ` element. Signatures are compared with `[string]::Equals(..., Ordinal)`, **not** `-cne`: PowerShell's `-ceq`/`-cne` are case-sensitive but still culture-sensitive, and linguistic collation treats control and format characters (U+001F, U+00AD, U+200B, …) as ignorable, so two signatures differing only by such a character compare equal and a real conflict is missed. Names are compared case-insensitively (as the registry does) but string `data` case-sensitively (remediation writes the literal). The redundant case only warns — rejecting it would break configs that work today. All of this lives in one function, `Assert-ValidConfig` (canonical enums in `$script:ValidRegistryTypes` / `$script:ValidComparisonOperators`): `Get-Configuration` calls it at runtime load, and `New-IntunePackage.ps1` dot-sources the engine and calls the same function for its package-time pre-flight — single source of truth, no mirrored copy to drift. Detection wraps each path's evaluation in a try/catch (mirroring remediation), so a value that slips past validation is marked non-compliant instead of aborting the whole run with a ConfigError.
